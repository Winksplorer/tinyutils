...
; when there is an argv[1], reboot. We don't really care if it's -r or not, just reboot because strcmp is too complex 😭
argv1_exists:
    ; sync disks then shut down
    _syscall 1, 1, sync_msg, 17                      ; write(2) (syncing disks message)
    _syscall 0xa2, 0, 0, 0                           ; sync(2)
    _syscall 1, 1, reboot_msg, 13                    ; write(2) (shutting down message)
    _syscall 0xa9, 0xfee1dead, 0x28121969, 0x1234567 ; reboot(2) (shutdown)
    _syscall 0x3c, 1, 0, 0                           ; exit(2) (if program is still executing (it should not be), then throw error)
